...
def run_python_code(code, test_code, timeout=30):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        return {"success": result.returncode == 0, "stdout": result.stdout, "stderr": result.stderr, "language": "python"}
...
def run_cpp_code(code, test_code, timeout=30):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        return {"success": result.returncode == 0, "stdout": result.stdout, "stderr": result.stderr, "language": "cpp"}
...
def run_java_code(code, test_code, timeout=30):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        return {"success": result.returncode == 0, "stdout": result.stdout, "stderr": result.stderr, "language": "java"}
...
def run_code(code, test_code, language="python", timeout=30):
    ...
    if language not in runners:
        return {"success": False, "stdout": "", "stderr": f"Unsupported language: {language}", "language": language}
...
print("Done! runner.py written successfully")
